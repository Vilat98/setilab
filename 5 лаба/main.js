...
document.querySelectorAll('.vlog').forEach(vlog => {

  card.onclick = e => {
      e.preventDefault()

      const price = Number(vlog.querySelector('.btn-primary').getAttribute('data-price')) // преобразует цену из дата прайс в число
      const name = vlog.querySelector('.vlog-text').innerText
